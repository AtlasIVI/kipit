    ...
    <div style={styles.root}>
      ...
      <div style={styles.container}>
        ...
        <div style={styles.headlineWrap}>
          <h1 style={styles.headline}>Bon retour.</h1>
          <p style={styles.sub}>Votre argent vous attend.</p>
        </div>
        ...
        <form onSubmit={handleSubmit} style={styles.form}>
          ...
          <button type="submit" disabled={loading} style={{ ...styles.submitBtn, opacity: loading ? 0.6 : 1 }}>
            {loading ? 'Connexion...' : <span style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', justifyContent: 'center' }}>Se connecter <ArrowRight size={18} /></span>}
          </button>
        </form>

        <p style={styles.footerText}>
          Vous avez une invitation ?{' '}
          <Link to="/register" style={styles.footerLink}>Créer un compte</Link>
        </p>
      </div>
    </div>
  )
...
const styles = {
  ...
  blob1: { position: 'absolute', top: '-15%', right: '-10%', width: '500px', height: '500px', borderRadius: '50%', background: 'radial-gradient(circle, rgba(34,197,94,0.12) 0%, transparent 70%)', pointerEvents: 'none' },
  blob2: { position: 'absolute', bottom: '-20%', left: '-15%', width: '600px', height: '600px', borderRadius: '50%', background: 'radial-gradient(circle, rgba(34,197,94,0.06) 0%, transparent 70%)', pointerEvents: 'none' },
  container: { width: '100%', maxWidth: '400px', position: 'relative', zIndex: 1, animation: 'fadeUp 0.5s ease both' },
  logoWrap: { display: 'flex', alignItems: 'center', gap: '0.6rem', marginBottom: '2.5rem' },
  ...
  headlineWrap: { marginBottom: '2rem' },
  headline: { fontSize: '2.4rem', fontWeight: '700', color: '#f1f5f9', letterSpacing: '-0.04em', margin: '0 0 0.3rem 0', fontFamily: '"Sora", "DM Sans", sans-serif', lineHeight: 1.1 },
...
styleTag.textContent = `
  ...
  input:-webkit-autofill { -webkit-box-shadow: 0 0 0 100px #0f1f2e inset !important; -webkit-text-fill-color: #f1f5f9 !important; }
`
if (!document.getElementById('auth-styles')) { styleTag.id = 'auth-styles'; document.head.appendChild(styleTag) }